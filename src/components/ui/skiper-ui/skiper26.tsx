...
export const useThemeToggle = ({
  ...
  const toggleTheme = useCallback(() => {
    ...
    document.startViewTransition(() => applyTheme(newIsDark));
  ...
  const setCrazyLightTheme = useCallback(() => {
    ...
    updateStyles(animation.css);
    if (typeof window === "undefined") return;
    if (!document.startViewTransition) { applyTheme(false); return; }
    document.startViewTransition(() => applyTheme(false));
  ...
  const setCrazyDarkTheme = useCallback(() => {
    ...
    updateStyles(animation.css);
    if (typeof window === "undefined") return;
    if (!document.startViewTransition) { applyTheme(true); return; }
    document.startViewTransition(() => applyTheme(true));
  }, [variant, start, blur, gifUrl, updateStyles]);
...
export const createAnimation = (
  ...
  if (variant === "circle" && start === "center") {
    return {
      name: `${variant}-${start}${blur ? "-blur" : ""}`,
      css: `::view-transition-group(root){animation-duration:0.7s;animation-timing-function:var(--expo-out)}::view-transition-new(root){animation-name:reveal-light${blur ? "-blur" : ""};${blur ? "filter:blur(2px);" : ""}}::view-transition-old(root),.dark::view-transition-old(root){animation:none;z-index:-1}.dark::view-transition-new(root){animation-name:reveal-dark${blur ? "-blur" : ""};${blur ? "filter:blur(2px);" : ""}}@keyframes reveal-dark${blur ? "-blur" : ""}{from{clip-path:circle(0% at 50% 50%)}to{clip-path:circle(100% at 50% 50%)}}@keyframes reveal-light${blur ? "-blur" : ""}{from{clip-path:circle(0% at 50% 50%)}to{clip-path:circle(100% at 50% 50%)}}`,
    };
  }
  ...
  if (variant === "circle" && start !== "center") {
    ...
    return {
      name: `${variant}-${start}${blur ? "-blur" : ""}`,
      css: `::view-transition-group(root){animation-duration:1s;animation-timing-function:var(--expo-out)}::view-transition-new(root){animation-name:reveal-light-${start}${blur ? "-blur" : ""};${blur ? "filter:blur(2px);" : ""}}::view-transition-old(root),.dark::view-transition-old(root){animation:none;z-index:-1}.dark::view-transition-new(root){animation-name:reveal-dark-${start}${blur ? "-blur" : ""};${blur ? "filter:blur(2px);" : ""}}@keyframes reveal-dark-${start}${blur ? "-blur" : ""}{from{clip-path:circle(0% at ${clipPos})}to{clip-path:circle(150% at ${clipPos})}}@keyframes reveal-light-${start}${blur ? "-blur" : ""}{from{clip-path:circle(0% at ${clipPos})}to{clip-path:circle(150% at ${clipPos})}}`,
    };
  }

  return {
    name: `${variant}-${start}${blur ? "-blur" : ""}`,
    css: `::view-transition-group(root){animation-timing-function:var(--expo-in)}::view-transition-new(root){mask:url('${svg}') ${start.replace("-", " ")} / 0 no-repeat;mask-origin:content-box;animation:scale-${start}${blur ? "-blur" : ""} 1s;transform-origin:${transformOrigin}}::view-transition-old(root),.dark::view-transition-old(root){animation:scale-${start}${blur ? "-blur" : ""} 1s;transform-origin:${transformOrigin};z-index:-1}@keyframes scale-${start}${blur ? "-blur" : ""}{to{mask-size:2000vmax}}`,
  };
};
